...
Controller
        |
        v
Model  <---->  Central Grade Repository
        |
        v
View
```
...
Lexical Analyzer --> Syntax Analyzer --> Semantic Analyzer --> Code Generator
        |                  |                  |
        v                  v                  v
        Symbol Table / Syntax Tree Repository
```
...
Video Service / Recommendation Service
          |
          v
Video Metadata Repository / User Repository
```
...
```text
Sensors / Cameras / Traffic Lights
              |
              v
Collect Data --> Clean Data --> Analyze Congestion --> Recommend Actions
              |                         |
              v                         v
        Traffic Data Repository     Control Logic Layer
                                            |
                                            v
...
Search Service | Booking Service | Payment Service
        |              |                 |
        v              v                 v
External Flight/Hotel/Car Providers      Payment Gateway
        |
        v
Booking / Customer / Payment Repository
        |
        v
Booking Pipeline: Validate --> Reserve --> Pay --> Confirm
```
